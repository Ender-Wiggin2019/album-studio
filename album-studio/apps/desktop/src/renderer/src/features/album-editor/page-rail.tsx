...
export function PageRail(): React.JSX.Element {
  ...
  const addBlankPage = useStudioStore((state) => state.addBlankPage)
  const removePage = useStudioStore((state) => state.removePage)
  const reorderPage = useStudioStore((state) => state.reorderPage)
  const [pendingDelete, setPendingDelete] = useState<string | null>(null)

  if (!project) return <aside />
  return (
          ...
          <div key={page.id} className="page-rail-item group">
            ...
            <div className="flex min-w-0 items-center justify-between gap-1">
              ...
              >
                {page.kind === 'cover'
                  ? '封面'
                  : `第 ${index} 页 · ${page.slots.filter((slot) => slot.assetId).length} 张`}
              </button>
              {page.kind === 'content' ? (
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (pendingDelete) removePage(pendingDelete)
